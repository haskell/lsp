...
runSessionWithHandles' serverProc serverIn serverOut config' caps rootDir session = do
  ...
  let initializeParams =
        L.InitializeParams
          { workDoneToken = Nothing
          , -- Narrowing to Int32 here, but it's unlikely that a PID will
            -- be outside the range
            processId = InL $ fromIntegral pid
          , clientInfo = Just lspTestClientInfo
          , locale = Nothing
          , rootPath = Just (InL $ T.pack absRootDir)
          , rootUri = InL $ L.filePathToUri absRootDir
          , capabilities = caps
          , -- TODO: make this configurable?
            initializationOptions = Just $ Object $ lspConfig config'
          , trace = Just L.TraceValue_Off
          , workspaceFolders = InL <$> initialWorkspaceFolders config
          }
...
getCodeActions doc range = do
  ...
  case rsp.result of
    ...
    Right (InR _) -> return []
    Left error -> throw (UnexpectedResponseError (fromJust rsp.id) error)
...
getCurrentDiagnostics doc = Map.findWithDefault [] (L.toNormalizedUri doc.uri) . curDiagnostics <$> get
...
resolveCodeAction ca = do
  ...
  case rsp.result of
    Right ca -> return ca
    Left er -> throw (UnexpectedResponseError (fromJust rsp.id) er)
...
-}
getResponseResult :: (Show (L.ErrorData m)) => L.TResponseMessage m -> L.MessageResult m
getResponseResult rsp =
  case rsp.result of
    Right x -> x
    Left err -> throw $ UnexpectedResponseError (fromJust rsp.id) err
...
getInlayHints :: L.TextDocumentIdentifier -> L.Range -> Session [L.InlayHint]
getInlayHints tId range = do
  rsp <- request SMethod_TextDocumentInlayHint (L.InlayHintParams Nothing tId range)
  pure $ L.absorbNull $ getResponseResult rsp

{- | Returns the inlay hints in the specified range, resolving any with
 a non empty _data_ field.
-}
getAndResolveInlayHints :: L.TextDocumentIdentifier -> L.Range -> Session [L.InlayHint]
getAndResolveInlayHints tId range = do
  inlayHints <- getInlayHints tId range
  for inlayHints $ \inlayHint -> if isJust inlayHint.data_ then resolveInlayHint inlayHint else pure inlayHint

-- | Resolves the provided inlay hint.
resolveInlayHint :: L.InlayHint -> Session L.InlayHint
...
resolveRequestWithListResp ::
  ...
  (Show (L.ErrorData m), L.MessageResult m ~ ([a] |? L.Null)) =>
  SMethod m ->
  L.MessageParams m ->
  Session [a]
