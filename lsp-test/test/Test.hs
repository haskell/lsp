...
main = hspec $ around withDummyServer $ do
  ...
  describe "text document VFS" $ do
    it "sends back didChange notifications (documentChanges)" $ \(hin, hout) ->
      runSessionWithHandles hin hout def fullCaps "." $ do
        ...
        let args = toJSON (VersionedTextDocumentIdentifier (doc ^. J.uri) beforeVersion)
  ...
  describe "getCodeActions" $
    it "works" $ \(hin, hout) -> runSessionWithHandles hin hout def fullCaps "." $ do
      ...
      liftIO $ action ^. J.title `shouldBe` "Delete this"
  ...
  describe "getAllCodeActions" $
    it "works" $ \(hin, hout) -> runSessionWithHandles hin hout def fullCaps "." $ do
      ...
      liftIO $ do
        ...
        action ^. J.title `shouldBe` "Delete this"
        action ^. J.command . _Just . J.command  `shouldBe` "deleteThis"
  ...
  describe "getDocumentSymbols" $
    it "works" $ \(hin, hout) -> runSessionWithHandles hin hout def fullCaps "." $ do
      ...
      liftIO $ do
        mainSymbol ^. J.name `shouldBe` "foo"
        mainSymbol ^. J.kind `shouldBe` SymbolKind_Object
        mainSymbol ^. J.range `shouldBe` mkRange 0 0 3 6
  ...
  describe "getCompletions" $
    it "works" $ \(hin, hout) -> runSessionWithHandles hin hout def fullCaps "." $ do
      ...
      let item = head comps
      liftIO $ item ^. J.label `shouldBe` "foo"
  ...
  describe "waitForDiagnosticsSource" $
    it "works" $ \(hin, hout) -> runSessionWithHandles hin hout def fullCaps "." $ do
      ...
      liftIO $ do
        diag ^. J.severity `shouldBe` Just DiagnosticSeverity_Warning
        diag ^. J.source `shouldBe` Just "dummy-server"
  ...
  describe "call hierarchy" $ do
    ...
    it "prepare works" $ \(hin, hout) -> runSessionWithHandles hin hout def fullCaps "." $ do
      ...
      liftIO $ head rsp ^. J.range `shouldBe` Range (Position 2 3) (Position 4 5)
  ...
  describe "semantic tokens" $ do
    it "full works" $ \(hin, hout) -> runSessionWithHandles hin hout def fullCaps "." $ do
      ...
      liftIO $ toks ^. J.data_ `shouldBe` [0,1,2,1,0]
