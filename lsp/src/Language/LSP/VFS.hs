...
virtualFileText vf = Rope.toText (file_text vf)

virtualFileVersion :: VirtualFile -> Int32
virtualFileVersion vf = lsp_version vf
...
applyDeleteFile logger (J.DeleteFile _ann _kind (J.toNormalizedUri -> uri) options) = do
  ...
  case old of
    ...
    Nothing
      | options ^? _Just . #ignoreIfNotExists . _Just /= Just True ->
          logger <& CantRecursiveDelete uri `WithSeverity` Warning
    _ -> pure ()
    ...
    Just vf ->
      let tfn = virtualFileName dir uri vf
          action = do
            ...
            unless exists $ do
              let contents = Rope.toText (file_text vf)
                  writeRaw h = do
